...
char* indexes(const char* path)
{
    // allocates memory for the 2 potential return paths
    char* php_index = malloc(strlen(path) + strlen("/index.php") + 1);
    char* html_index = malloc(strlen(path) + strlen("/index.html") + 1);
    
    // check if memory allocation succeeded
    if (php_index == NULL || html_index == NULL)
    {
        printf("Unsuccessful memory allocation.\n");
        return 0;
    }
    
    // places the return path into their respective char*s
    strcpy(php_index, path);
    strcpy(html_index, path);
    strcat(php_index, "/index.php");
    strcat(html_index, "/index.html");
    
    // if index.php exists, the corresponding path will be returned and the html
    // path will be free'd
    if (access(php_index, F_OK) == 0)
    {
        free(html_index);
        return php_index;
    }
    
    // if index.php doesn't exist but index.html does, the latter path will be
    // returned and the php path will be free'd
    else if (access(html_index, F_OK) == 0)
    {
        free(php_index);
        return html_index;
    }
    
    // if neither exist, free both malloc'd indexes and return NULL
    else
    {
        free(html_index);
        free(php_index);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // a buffer to store read bytes
    BYTE buffer[BYTES];
    
    // sets the initial length to be 0
    *length = 0;
    
    // sets the content pointer initially to NULL so realloc will work
    *content = NULL;
    
    // iterates through the file until the end of the file is reached
    while(true)
    {
        // reads BYTES bytes from the file into the buffer and notes the number
        // of bytes that were successfully read
        size_t bytes_read = fread(buffer, sizeof(BYTE), BYTES, file);
        
        // if any bytes were read, *content will grow by that many to accomodate
        // the copying of those bytes into memory
        if (bytes_read > 0)
        {
            *content = realloc(*content, *length + bytes_read);
            if (*content == NULL)
            {
...
const char* lookup(const char* path)
{
    // sets the extension to everything after the "."
    char* ext = strchr(path, '.');
    
    // checks what the extension is and returns the respective MIME type
...
bool parse(const char* message, char* abs_path, char* query)
{
    ...
    // checks if "SP/"" (indicating the start of a request-target) follows
    if (strncmp(message + 3, " /", 2) != 0)
    {
        error(501);
        return false;
    }
    
    // sets the start of a the abs_path as the / and its end to be the next SP
    char* path_start = strchr(message, '/');
    char* path_end = strchr(path_start, ' ');
    
    // sets the query to be empty at first
    strcpy(query, "");
    
    // if a ? exists, then there must be a query
    // no query yeilds a "length" of -1 (necessary for my 505 error check below)
    int querylen = -1;
    if (strchr(message, '?') != NULL)
    {
        // the query starts right after the ?
        char* query_start = strchr(message, '?') + 1;
        
        // the query ends at the SP after the path_start
        char* query_end = path_end;
        
        // the abs_path will now end at the ?
        path_end = query_start - 1;
        
        // the query length is the difference between the start and end pointers
        querylen = query_end - query_start;
        
        // the query is copied to query
        strncpy(query, query_start, querylen);
        query[querylen] = '\0';
    }
    
    // the abs_path length is the difference between its start and end
    int pathlen = path_end - path_start;
    
    // the abs_path is copied to abs_path
    strncpy(abs_path, path_start, pathlen);
    abs_path[pathlen] = '\0';
    
    // if there is an " anywhere in the message or a ? in the abs_path, it must
    // be a bad request
    if (strchr(message, '"') != NULL || strchr(abs_path, '?') != NULL)
    {
        error(400);
        return false;
    }
    
    // checks to see if the correct HTML version is used at the end
    // addition "pushes" past the method and R-T to the end of the request-line
    if (strncmp(message + 5 + pathlen + querylen, " HTTP/1.1", 9) != 0)
    {
        error(505);
        return false;
    }
